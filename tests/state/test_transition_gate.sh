...
cp "$SRC/run/events.jsonl" "$WORK/events.jsonl"
...
state() {
  ECOSYSTEM_STORE="$WORK" python3 "$SRC/tools/state.py" "$@"
}
...
task_status() {
  ECOSYSTEM_STORE="$WORK" python3 - "$SRC" "$1" <<'PY'
import os, sys
...
state verify | tail -n 1 | grep -qx 'status: DONE' || fail 'historical chain failed'
[ "$(status_of 'done')" -eq 2 ] || fail 'historical done count changed'
[ "$(status_of ready)" -eq 2 ] || fail 'historical ready count changed'
[ "$(status_of planned)" -eq 77 ] || fail 'historical planned count changed'
