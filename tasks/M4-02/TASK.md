# M4-02 Capsule `session.send` and `events`
...
Capsule `session.send` and `events`.

## Scope

Add `ClientMsg::SessionSend { session, text }` and an `events` subscription (agent state transitions, exit, last activity) to the capsule control protocol; host-side client in the daemon. Gap 3; terminal-observation research design reused.
...
> protocol round-trip and in-process capsule session tests (no Docker)

Host part (run by the host Claude Code session, D-061):

> the integration test that sends text into a running instance and observes the `Working` transition on the event stream runs on the host Docker from the checkout in `tasks/M1-02/checkout.txt`

When a container part exists the host part first asserts that
`tasks/M4-02/verify.container.out` ends with `status: DONE`, so a
passing host part can never mask a failed container part (D-086).

## Evidence expected (D-118)

- `tasks/M4-02/verify.container.out` (container part, containing `status: DONE`)
