# M8-02 Pull request open and update
...
Pull request open and update.

## Scope

Push the branch, open or update the PR titled with the issue identifier, add the PR URL to the issue (`addedExternalUrls`), mark ready after M7 success.
...
- [ ] container check passes: `cargo nextest run -p jackin-daemon github::pr`
- [ ] container check passes: `gh`
- [ ] host check passes: `tail -n1 tasks/M8-02/verify.container.out`
...
> `cargo nextest run -p jackin-daemon github::pr` passes against a recording `gh`: one PR create and one update for the same branch, and the issue write carries the PR URL

Host part (run by the host Claude Code session, D-061):

> `tail -n1 tasks/M8-02/verify.container.out` is `status: DONE` and the file is committed as the task's evidence

When a container part exists the host part first asserts that
`tasks/M8-02/verify.container.out` ends with `status: DONE`, so a
passing host part can never mask a failed container part (D-086).

## Evidence expected (D-118)

- `tasks/M8-02/verify.container.out` (container part, containing `status: DONE`)
- `tasks/M8-02/verify.container.out` (container part)
