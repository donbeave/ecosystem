# M12-01 Remote daemon transport
...
Reachable daemon interface from another host (gap 10; jackin's `jackin-remote` research as input).
...
> `cargo nextest run -p jackin-daemon remote::` passes: the transport round-trip against an in-process server, with an unreachable peer surfaced as an error and not a silent success
