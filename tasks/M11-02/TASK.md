# M11-02 Publish the `crew` role images
...
> `jackin role published-image .` in each role checkout prints `docker.io/donbeave/jackin-crew-<p>:latest`, `! grep -rqE 'velnor|self-hosted' .github/workflows` holds in each, and both architectures use the same apt "chromium" path in the operator image (D-077)
...
- `tasks/M11-02/verify.container.out` (container part, containing `status: DONE`)
