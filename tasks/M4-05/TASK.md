# M4-05 Runtime matrix for prompt delivery and block detection
...
> per-runtime entrypoint tests

Host part (run by the host Claude Code session, D-061):

> matrix table in `tasks/M4-05/` with six rows, each backed by a capture file for delivery and one for block detection; the M4-02 event stream shows `Blocked` then `Working` for every runtime that has a credential

When a container part exists the host part first asserts that
`tasks/M4-05/verify.container.out` ends with `status: DONE`, so a
passing host part can never mask a failed container part (D-086).

## Evidence expected (D-118)

- `tasks/M4-05/verify.container.out` (container part, containing `status: DONE`)
