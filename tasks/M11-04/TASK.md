...
Same issue class as M8-03 (scratch repository of `tasks/M3-07/scratch-repo.txt`) executed on the server; evidence in `tasks/M11-04/`.
...
> `jackin daemon status --format json` on the server lists the issue, its output is filed as `tasks/M11-04/status.json`, and `jq -e '.[0].host'` on that file names the server

## Evidence expected (D-118)

- `tasks/M11-04/status.json` (host part)

## Proof (browser/attach)

An issue assigned from the laptop runs on the server; the M5-05 external URL names the host.
