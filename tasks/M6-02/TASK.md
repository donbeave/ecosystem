...
- [ ] container check passes: `cargo nextest run -p jackin-daemon linear::writeback`
- [ ] host check passes: `tail -n1 tasks/M6-02/verify.container.out`
...
> `cargo nextest run -p jackin-daemon linear::writeback` passes against the recording fake: one plan replacement and one read-modify-write `issueUpdate` per tick, a replayed tick writes nothing, and a tick whose `- [ ]` line is absent from the freshly read text aborts

Host part (run by the host Claude Code session, D-061):

> `tail -n1 tasks/M6-02/verify.container.out` is `status: DONE` and the file is committed as the task's evidence

When a container part exists the host part first asserts that
`tasks/M6-02/verify.container.out` ends with `status: DONE`, so a
passing host part can never mask a failed container part (D-086).

## Evidence expected (D-118)

- `tasks/M6-02/verify.container.out` (container part, containing `status: DONE`)
- `tasks/M6-02/verify.container.out` (container part)
