...
Least-loaded placement, previous-host preference on retry, wait when saturated, no duplicate execution (D-026). Only the manager host polls Linear; server daemons receive candidates over the M12-01 transport, so the one app-user rate budget (5,000 requests per hour) is drawn by one poller (D-087).
...
- [ ] container check passes: `cargo nextest run -p jackin-daemon placement::`
- [ ] host check passes: `tail -n1 tasks/M12-02/verify.container.out`
...
> `cargo nextest run -p jackin-daemon placement::` passes: least-loaded placement, previous-host preference on retry, a wait when saturated, no duplicate execution, and only the manager host polling the tracker

Host part (run by the host Claude Code session, D-061):

> `tail -n1 tasks/M12-02/verify.container.out` is `status: DONE` and the file is committed as the task's evidence

When a container part exists the host part first asserts that
`tasks/M12-02/verify.container.out` ends with `status: DONE`, so a
passing host part can never mask a failed container part (D-086).

## Evidence expected (D-118)

- `tasks/M12-02/verify.container.out` (container part, containing `status: DONE`)
- `tasks/M12-02/verify.container.out` (container part)
