...
Two real hosts, two scratch issues (`tasks/M3-07/scratch-repo.txt`), one host failure; evidence in `tasks/M12-03/`.
...
- [ ] `ledger.json` is filed in the task folder.
...
> `jq -e` over `tasks/M12-03/ledger.json` shows exactly one re-placement and one new attempt, and the two host names differ

## Evidence expected (D-118)

- `tasks/M12-03/ledger.json` (host part)

## Proof (browser/attach)

Both sessions show their host in `externalUrls`; the re-placed run shows the new host.
