# M3-06 Stop on non-active or terminal state
...
Stop on non-active or terminal state.

## Scope

When an issue leaves the active states or the delegate is removed, stop the container; on terminal state also remove the workspace (Symphony §8.5 adapted).
...
> stop-on-state-change unit test with a recorded state fixture
