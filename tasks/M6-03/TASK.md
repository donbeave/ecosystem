...
- [ ] host check passes: `grep -c pre-read`
...
> on `tasks/M6-03/daemon.log`, `grep -c issue.read` is 1; `grep -c pre-read` is 3 and each "pre-read" line immediately precedes a `write` line of kind "tick"; the `write` count equals 3 ticks plus the transitions and heartbeats in `writes.txt`; every remaining line is "poll" (D-081, D-087)

## Evidence expected (D-118)

- `tasks/M6-03/writes.txt` (host part)
- `tasks/M6-03/daemon.log` (host part)

## Proof (browser/attach)

`- [x]` ticks appear in the issue description and the session plan as the agent finishes items.
