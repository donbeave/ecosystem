...
- [ ] container check passes: `jackin load donbeave/crew-builder --agent claude`
- [ ] container check passes: `mise install`
- [ ] container check passes: `cargo nextest --version`
- [ ] container check passes: `cargo public-api --version`
...
> `jackin role validate` passes; `jackin load donbeave/crew-builder --agent claude` starts; inside: `mise install` in a termrock checkout is a no-op, `cargo nextest --version`, `cargo public-api --version`, `rustup run 1.97.1 cargo --version` exit 0; no `agent-browser`, no `op` on PATH
