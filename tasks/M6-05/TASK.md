# M6-05 Daemon lane fallback on quota exhaustion / stuck (D-057)
...
Daemon lane fallback on quota exhaustion / stuck (D-057).
...
- [ ] container check passes: `cargo nextest run -p jackin-daemon dispatch::fallback`
- [ ] host check passes: `tail -n1 tasks/M6-05/verify.container.out`
...
> `tail -n1 tasks/M6-05/verify.container.out` is `status: DONE` and the file is committed as the task's evidence

When a container part exists the host part first asserts that
`tasks/M6-05/verify.container.out` ends with `status: DONE`, so a
passing host part can never mask a failed container part (D-086).

## Evidence expected (D-118)

- `tasks/M6-05/verify.container.out` (container part, containing `status: DONE`)
- `tasks/M6-05/verify.container.out` (container part)
