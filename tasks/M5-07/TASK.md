...
Review M5 pull request.

## Scope

Non-blocking (D-055). Review state machine, heartbeat, and label maintenance for write-volume bounds (D-049: bounded by the state machine, not agent chatter) and for D-023 (no tracker credential in the container).
...
> A review by the configured reviewer login whose "commit_id" equals line 2 of `tasks/<id>/pr.txt` (or a later head that has it as an ancestor, D-091) with a `verdict:` line (D-079); checklist lines from the final message are appended to the issue by the host session (D-055)
...
`tasks/M5-07/verify.container.out` ends with `status: DONE`, so a
passing host part can never mask a failed container part (D-086).

## Evidence expected (D-118)

- `tasks/M5-07/verify.container.out` (container part, containing `status: DONE`)
