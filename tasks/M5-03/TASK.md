# M5-03 Stuck detection via the stall window
...
Stuck detection via the stall window.

## Scope

Compute "stuck" (D-021; default 5-minute stall window, configurable): no capsule activity within the window while in `working` → transition to `stuck` with an activity naming the window and the last progress time; activity resumes → back to `working`. Recovery (kill and retry) stays in M7-02; here the state is only surfaced.
...
> Fake-clock test: a silent session crosses the window and emits one "stuck" transition; a resumed session emits one "working" transition; no flapping inside the window
...
`tasks/M5-03/verify.container.out` ends with `status: DONE`, so a
passing host part can never mask a failed container part (D-086).

## Evidence expected (D-118)

- `tasks/M5-03/verify.container.out` (container part, containing `status: DONE`)
