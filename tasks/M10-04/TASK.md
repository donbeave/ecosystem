# M10-04 Console fleet route with attach
...
| --- | --- |
| milestone | M10 |
| depends on | M10-01, M10-02, M10-03 |
| role | `the-architect` |
| lane | L1 |
| runtime | claude |
| fallback lane | L2 |
...
Console fleet route with attach.

## Scope

New console route reading M10-01; one key attaches via the capsule; built only from termrock widgets (D-006).
...
- [ ] container check passes: `cargo nextest run -p jackin-tui fleet::`
- [ ] container check passes: `grep -rq`
- [ ] host check passes: `tail -n1 tasks/M10-04/verify.container.out`
...
> `cargo nextest run -p jackin-tui fleet::` passes: the route lists the runs from a fixture snapshot, and `grep -rq` finds no local copy of the new termrock widgets in the console crate (D-006)

Host part (run by the host Claude Code session, D-061):

> `tail -n1 tasks/M10-04/verify.container.out` is `status: DONE` and the file is committed as the task's evidence

When a container part exists the host part first asserts that
`tasks/M10-04/verify.container.out` ends with `status: DONE`, so a
passing host part can never mask a failed container part (D-086).

## Evidence expected (D-118)

- `tasks/M10-04/verify.container.out` (container part, containing `status: DONE`)
- `tasks/M10-04/verify.container.out` (container part)
