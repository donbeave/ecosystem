...
system and the on-disk tree is the local mirror.

One folder per roadmap task id directly under `tasks/` (D-038), as generated
today:

```text
<repo>/
  tasks/
    README.md                    # one row per task id: Task | Status (AGENTS.md)
    M1-05a/
      TASK.md                    # objective, scope, references, definition of done
      ...
      refs/
        sources.txt              # the documents staged into the bundle (D-086)
      # written at run time:
      evidence.json              # what it actually produced (D-110, D-118)
      verify.out                 # captured output of verify.sh; last line is the status
      attempts.log               # one line per attempt, epoch lines (D-084)
    M1-05b/
      ...
  verify.sh                      # root oracle: the run-level status line (D-069)
```
