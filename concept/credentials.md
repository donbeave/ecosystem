...
| 5a | GitHub: push branches, open/update PRs, read repos in the `jackin-project` organization | GitHub App installation (preferred) or fine-grained PAT with `contents:write`, `pull_requests:write`, `metadata:read` | `github-app-jackin-daemon-jackin-project`, fields exactly `app id`, `client id`, `installation id`, `PEM private key` (lower-case labels) — the one GitHub App owned by `jackin-project`, installed on the `jackin-project` organization | `op://jackin/github-app-jackin-daemon-jackin-project/app id`, `…/client id`, `…/installation id`, `…/PEM private key` | runtime | **PARTIAL** — `tailrocks/GitHub App — jackin-package-updater` and `…tailrocks-package-updater` exist but are scoped to package-update repos and permissions; a daemon app (or widened installation) is needed. Repo-scope PATs on `Private/GitHub` are tap-publisher/renovate/read-only, not PR-management |
| 5b | GitHub: push branches, open/update PRs, read repos in the `tailrocks` organization | GitHub App installation (preferred) or fine-grained PAT with `contents:write`, `pull_requests:write`, `metadata:read` | `github-app-jackin-daemon-tailrocks`, fields exactly `app id`, `client id`, `installation id`, `PEM private key` (lower-case labels) — the one GitHub App owned by `jackin-project`, installed on the `tailrocks` organization | `op://jackin/github-app-jackin-daemon-tailrocks/app id`, `…/client id`, `…/installation id`, `…/PEM private key` | runtime | **PARTIAL** — `tailrocks/GitHub App — jackin-package-updater` and `…tailrocks-package-updater` exist but are scoped to package-update repos and permissions; a daemon app (or widened installation) is needed. Repo-scope PATs on `Private/GitHub` are tap-publisher/renovate/read-only, not PR-management |
...
| 7 | `gh` CLI in the daemon host / container (`GH_TOKEN`) | derived from #5a/#5b (installation token) or PAT | none extra if #5a/#5b are Apps; otherwise `github-daemon-pat` | `op://jackin/github-daemon-pat/token` | runtime | **PARTIAL** — covered by #5a/#5b once decided |
| 8 | Claude runtime in daemon-run containers | OAuth token (subscription) or `ANTHROPIC_API_KEY` | `claude-daemon` field `api key` (one field; the laptop OAuth-token mode of D-082 uses the same item) | `op://jackin/claude-daemon/api key` | runtime | **PARTIAL** — `Private/Claude (2026-04-27)` has `auth token`; `Private/Anthropic` and `Private/Claude (2026-03-20)` have API-key fields; nothing dedicated, nothing referenced. Today `auth_forward = "sync"` from the laptop |
...
| 11 | Kimi runtime | `KIMI_CODE_API_KEY` | copy into `jackin/kimi-daemon` field `api key` (the daemon service account reads only vault `jackin`, D-076) | `op://jackin/kimi-daemon/api key` | runtime | **EXISTS** (`ChainArgos/Kimi`, field `Test`); rename field from `Test` recommended |
| 12 | OpenCode runtime | API key | copy into `jackin/opencode-daemon` field `api key` (D-076) | `op://jackin/opencode-daemon/api key` | runtime | **EXISTS** (`Private/OpenCode`), not yet referenced |
...
| 15 | Pull role images / push locally built role images (Docker Hub `donbeave/jackin-crew-*`, GHCR) | registry token | `registry-dockerhub` fields `username`, `token` — token minted by user `donbeave` with Read & Write on namespace `donbeave` (ROADMAP M11-02 publishes `docker.io/donbeave/jackin-crew-<p>`; namespace confirmed by `curl -s -o /dev/null -w '%{http_code}' https://hub.docker.com/v2/users/donbeave/` = 200) (→ GitHub Actions secrets `DOCKERHUB_USERNAME`, `DOCKERHUB_TOKEN` in M11-02); GHCR via #5a/#5b | `op://jackin/registry-dockerhub/username`, `…/token` | runtime (pull) / setup (push) | **PARTIAL** — Docker Hub logins exist with a CI token field; public role images need no credential to pull; nothing named for jackin |
...
| 17 | Headless `op` for the daemon itself (server host, no desktop app) | 1Password service account token scoped to vault `jackin` | `op-service-account-jackin-daemon` (store in `tailrocks`, **not** in the vault it unlocks) | `OP_SERVICE_ACCOUNT_TOKEN` from `op://tailrocks/op-service-account-jackin-daemon/credential`, materialised once on the server by M11-03: the host session reads the reference and writes the value to `~/.config/jackin/daemon.env` there (`umask 077; ssh -i … 'cat > ~/.config/jackin/daemon.env'` fed from `op read` on stdin, never argv), and `jackin daemon install` loads the variable from that file. The vault `tailrocks` is unreadable from the server (a service account cannot read it and there is no desktop app there), so the value cannot be resolved at daemon start. jackin resolves every other `op://` reference by shelling out to `op`, so M11-03 also installs the `op` CLI on the server (direct download into `~/.local/bin`, no sudo) | runtime | **CREATE** — pattern exists (`ChainArgos/ruxel CI service account`, `…lightdash-migration-agent`) |
...
| 21 | Server hosts for M11 and M12 | address, SSH user, SSH private key | `server-host-1`, `server-host-2` (SSH Key items) fields `address`, `ssh user`, `private key` | `op://jackin/server-host-1/address`, `…/ssh user`, `…/private key`; same for `server-host-2` | setup (M11-03, M12-03) | **CREATE** by the human in preflight (D-076) |
| 22 | Operator service account for `jackin-exec` (vault `jackin` read + write) | 1Password service account token | `op-service-account-jackin-operator` in `tailrocks` (API Credential, field `credential`) | `op://tailrocks/op-service-account-jackin-operator/credential` | runtime (on-demand binding, M1-05d) | **CREATE** by the human in preflight (D-076) |

Counts: **EXISTS 6** (#4, #6, #11, #12, #14, #19; #14 counts three
provider keys as one row), **PARTIAL 8** (#5a, #5b, #7, #8, #9, #10, #13, #15),
...
| --- | --- |
| #1 client id/secret, #2 webhook secret, #3 workspace tokens, #5a/#5b/#7 GitHub App key, #8–#14 provider keys, #15 registry pull if private, #17 service account, #18 relay | #4 Linear SSO, #6 GitHub login + OTP for the browser profile, #16 signing (CI), #19 tap publish |
