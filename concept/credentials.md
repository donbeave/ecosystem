...
| 15 | Pull role images / push locally built role images (Docker Hub `donbeave/jackin-crew-*`, GHCR) | registry token | `registry-dockerhub` fields `username`, `token` — token minted by user `donbeave` with Read & Write on namespace `donbeave` (ROADMAP M11-02 publishes `docker.io/donbeave/jackin-crew-<p>`; namespace confirmed by `curl -s -o /dev/null -w '%{http_code}' https://hub.docker.com/v2/users/donbeave/` = 200) (→ GitHub Actions secrets `DOCKERHUB_USERNAME`, `DOCKERHUB_TOKEN` in M11-02); GHCR via #5 | `op://jackin/registry-dockerhub/username`, `…/token` | runtime (pull) / setup (push) | **PARTIAL** — Docker Hub logins exist with a CI token field; public role images need no credential to pull; nothing named for jackin |
