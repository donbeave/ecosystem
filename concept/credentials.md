...
| 15 | Pull role images / push locally built role images (Docker Hub `projectjackin/*`, GHCR) | registry token | `registry-dockerhub` fields `username`, `token` (→ GitHub Actions secrets `DOCKERHUB_USERNAME`, `DOCKERHUB_TOKEN` in M11-02); GHCR via #5 | `op://jackin/registry-dockerhub/username`, `…/token` | runtime (pull) / setup (push) | **PARTIAL** — Docker Hub logins exist with a CI token field; public role images need no credential to pull; nothing named for jackin |
