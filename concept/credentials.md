...
| 2 | Linear webhook verification (`Linear-Signature` HMAC) | signing secret | `linear-agent-app` field `webhook signing secret` | `op://jackin/linear-agent-app/webhook signing secret` | runtime (relay or daemon) | **CREATE** — created together with #1 |
...
| 18 | Webhook relay (Hookdeck / Cloudflare Worker / smee) for daemon behind NAT (Q-015) | relay API key or Worker deploy token | `webhook-relay` | `op://jackin/webhook-relay/token` | runtime | **CREATE** — depends on Q-015; `ChainArgos/Cloudflare` has an API token field if the Worker path is chosen |
