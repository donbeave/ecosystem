...
- **Event mapping:** `REQUEST_CHANGES` when at least one `blocking` finding remains; `COMMENT` when clean or only non-blocking findings. **Never `APPROVE`**: (a) the reviewer's `gh` identity in M2..M9 is the same GitHub App / PAT the daemon uses to open PRs (`concept/credentials.md` rows #5, #7), and GitHub rejects approving one's own PR; (b) merge authority belongs to the Linear "merging" state and M8-01, not to the reviewer. Each inline body starts with a severity tag `blocking:` / `major:` / `minor:` so `verify.sh` can grep for open `blocking` threads.
