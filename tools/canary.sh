...
mkdir -p "$DIR/refs"
cat >"$DIR/refs/sources.txt" <<EOF
# Sources for $TASK. Written by tools/canary.sh.
roadmap: none (synthetic off-graph canary)
format: synthetic readiness rehearsal
EOF
...
python3 tools/bundle.py hash --bundle-dir "$DIR" >"$TMP/hash.out"
...
mkdir -p "$WT/$DIR/refs"
cp "$DIR/refs/sources.txt" "$WT/$DIR/refs/sources.txt"
