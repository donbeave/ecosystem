...
Status: **PROPOSAL, 2026-08-27 — defaults adopted (D-053); finalize = the
human says so.** Every recommended answer this document depended on is the
working decision under D-050 and D-053, so task authoring may proceed;
"finalized" is a separate, explicit statement by the human. Task folders
under `tasks/` are created only after that statement (D-038).
...
- `host` tasks in this milestone: M1-02a (remove `jackin-preview`, branch
  ...
  front.
- Docker Desktop installed, running, and sized for two role containers plus
  a rootless DinD daemon; the host stays awake while agents run.
...
- The human's Linear account is a workspace admin (OAuth app creation in
  ...
  profile and need admin); the Linear team key that hosts the project and
  the test issues is recorded in `tasks/M1-09/` and `tasks/M1-12/`.
...
per issue; Docker Desktop limits high enough for `max_concurrent_agents =
2` plus DinD; the scratch GitHub repository for M3-07 is created by the
operator role with the forwarded `gh` identity, not by the human.
...
M5-03 → M5-06 → M5-07, with M1-08 → M1-09 → M1-12 joining at
...
Empty as of D-053: every row this section held had a recommended answer,
and D-050/D-053 adopt recommended answers as the working decision. The
adopted answers are stated in `SPEC.md` (issue convention §4, polling §5,
execution §6, visibility §7, ledger §8, credentials §9a, unattended
execution §9e) and in the concept documents. What remains is listed in
`OPEN-QUESTIONS.md` and blocks no task:

| Blocks | Question | Status |
| --- | --- | --- |
| — | Q-002 project name | Placeholder "the manager" until M10 names the console route. |
| — | Q-005 planner approval detail | D-028 adopted; how a batch of proposed issues is approved beyond assigning each one is undecided. |
| — | Q-009 delivery beyond Linear | D-029 and D-051 adopted; desktop and phone delivery beyond Linear's own apps is undecided. |
