...
| M10-03 | termrock: `TerminalPane` widget | Scrollback, follow, selection over `TerminalCellSource` with input-forwarding outcomes (§8). | termrock | M10-02 | crew-builder | L2 | goal | L | Story added and the flagship set extended; golden frames recorded by the agent, blessed by the host session under D-075 (`mise run bless-previews`, never by the container), committed, and the frame text filed in `tasks/M10-03/`; `mise run gate` green on the branch. | — |
